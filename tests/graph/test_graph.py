...
class TestTaskGraphBasic:
    ...
    def test_graph_fan_out(self):
        """扇出：一个节点到多个下游"""
        source = TaskExecutor("src", add_one, execution_mode="serial")
        sink_a = TaskExecutor("SinkA", double, execution_mode="serial")
        sink_b = TaskExecutor("SinkB", to_str, execution_mode="serial")

        graph = TaskGraph("test_graph_fan_out")
        graph.set_nodes(nodes=[source, sink_a, sink_b])
    ...
    def test_graph_fan_in(self):
        """扇入：多个上游到一个下游"""
        source_a = TaskExecutor("SrcA", add_one, execution_mode="serial")
        source_b = TaskExecutor("SrcB", double, execution_mode="serial")
        merge = TaskExecutor("merge", to_str, execution_mode="serial")

        graph = TaskGraph("test_graph_fan_in")
        graph.set_nodes(nodes=[source_a, source_b, merge])
...
class TestTaskGraphSnapshotCounts:
    def test_fan_in_upstream_counts(self):
        ...
        merge_snapshot = merge.get_snapshot()
        assert merge_snapshot["upstream_counts"] == {"src_a": 2, "src_b": 2}
        assert src_a.get_snapshot()["downstream_counts"] == {"merge": 2}
        assert src_b.get_snapshot()["downstream_counts"] == {"merge": 2}
    ...
    def test_fan_out_downstream_counts(self):
        ...
        src_snapshot = src.get_snapshot()
        assert src_snapshot["downstream_counts"] == {"sink_a": 2, "sink_b": 2}
        assert sink_a.get_snapshot()["upstream_counts"] == {"src": 2}
        assert sink_b.get_snapshot()["upstream_counts"] == {"src": 2}
...
class TestTaskGraphAsync:
    ...
    async def test_graph_async_fan_out(self):
        """async 模式：扇出"""
        source = TaskExecutor("src", async_add_one, execution_mode="async")
        sink_a = TaskExecutor("sink_a", async_double, execution_mode="async")
        sink_b = TaskExecutor("sink_b", async_to_str, execution_mode="async")
    ...
    async def test_graph_async_fan_in(self):
        """async 模式：扇入"""
        source_a = TaskExecutor("src_a", async_add_one, execution_mode="async")
        source_b = TaskExecutor("src_b", async_double, execution_mode="async")
        merge = TaskExecutor("merge", async_to_str, execution_mode="async")
...
class TestTaskGraphStructure:
    ...
    def test_cross_structure(self):
        """TaskCross：分层结构全连接"""
        layer1 = [TaskExecutor(f"L1{i}", add_one) for i in range(2)]
        layer2 = [TaskExecutor(f"L2{i}", double) for i in range(3)]
    ...
    def test_grid_structure(self):
        """TaskGrid：网格结构正确连接"""
        grid = [[TaskExecutor(f"g{i}{j}", add_one) for j in range(2)] for i in range(2)]
...
class TestTaskGraphAnalysis:
    ...
    def test_getters_refresh_analysis_after_connect(self):
        ...
        initial_sources = set(graph.get_source_nodes())
...
class TestNodeExecutionMatrix:
    ...
    def test_serial_serial(self):
        """测试串行图模式 + 串行执行模式"""
        s1 = TaskExecutor("s1", add_one, execution_mode="serial")
        s2 = TaskExecutor("s2", double, execution_mode="serial")
    ...
    def test_serial_thread(self):
        """测试串行图模式 + 线程池执行模式"""
        s1 = TaskExecutor("s1", add_one, execution_mode="thread", max_workers=4)
        s2 = TaskExecutor("s2", double, execution_mode="thread", max_workers=4)
    ...
    async def test_async_async(self):
        """测试异步图模式 + 异步执行模式"""
        s1 = TaskExecutor("s1", async_add_one, execution_mode="async", max_workers=4)
        s2 = TaskExecutor("s2", async_double, execution_mode="async", max_workers=4)
...
class TestTaskGraphThread:
    ...
    def test_graph_thread_schedule(self):
        ...
        s1 = TaskExecutor("s1", add_one, execution_mode="serial")
        s2 = TaskExecutor("s2", double, execution_mode="serial")
        s3 = TaskExecutor("s3", to_str, execution_mode="serial")
...
class TestSourceNodes:
    def test_source_nodes_linear(self):
        ...
        source_names = graph.get_source_nodes()
