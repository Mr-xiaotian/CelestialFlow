...
| 事件 | 触发时机 | 参数 |
|------|----------|------|
| `on_start` | 执行器开始运行 | 无（任务总数通过 `on_task_added` 通知） |
| `on_task_success` | 任务成功 | `count`: 成功数量（默认 1） |
| `on_task_fail` | 任务失败 | `count`: 失败数量（默认 1） |
| `on_task_duplicate` | 检测到重复任务计数 | `count`: 重复数量（默认 1） |
| `on_task_added` | 新任务经由 `put_task` 加入队列 | `count`: 新增任务数 |
| `on_finish` | 执行器结束运行 | 无 |
| `observer_error` | 观察者回调抛出异常时 | `method_name`: 异常回调名, `exception`: 捕获的异常 |

### 自动异常包装机制

`BaseObserver` 通过 `__init_subclass__` 在子类创建时自动包装所有覆写的回调方法（`on_start`、`on_task_success`、`on_task_fail`、`on_task_duplicate`、`on_task_added`、`on_finish`）：
...
- `TaskMetrics.on_start()` → 广播 `on_start`（由 `BaseTaskNode._prepare_start()` 调用）
- `TaskMetrics.add_external_input_count(count)` → 广播 `on_task_added`
...
观察者通过 `node.add_observer(observer)` 注册（内部存入 `TaskMetrics._observers`）。当 observer 列表为空时，广播循环为空操作。
...
| 类 | 所在文件 | 说明 |
|---|---------|------|
| `PrintObserver` | `core_observer_print.py` | 基于 `print` 的控制台观察者，构造时需传入 `name` 前缀，维护 `total` / `succeeded` / `failed` / `duplicated` 计数 |

> 用户也可按需继承 `BaseObserver` 实现自定义观察者。详见 `docs/zh-CN/src/observability/core_observer_print.md`。
