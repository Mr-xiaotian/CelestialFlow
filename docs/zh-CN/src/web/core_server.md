...
用于 Web UI 获取最新数据。大部分拉取接口（`pull_status`、`pull_structure`、`pull_errors`、`pull_analysis`）支持 `known_rev` 机制：若服务端数据版本未变，则返回 `data: null` 以节省带宽。`pull_config`、`pull_task_injection`、`pull_server_state` 不使用 `known_rev` 机制，每次均返回完整数据。
...
| `pull_analysis` | `dict[str, Any]` | 获取图的拓扑分析结果 |
| `pull_task_injection` | `dict[str, list[Any]]` | 供 TaskGraph 拉取待注入的任务队列（按节点名分组，读取后清空） |
| `pull_server_state` | `dict[str, Any]` | 获取 Reporter 同步所需的服务端状态（interval/is_current_graph/has_structure/max_event_id_in_fail） |
...
> ⚠️ **已变更**：新增 `graph_id` 字段，用于 Reporter 端 graph 上下文校验。
...
> ⚠️ **已变更**：错误模型由 `ErrorsMetaModel`/`ErrorsContentModel` 双模型合并为单一 `ErrorsModel`，不再区分元信息和内容。错误数据直接写入 SQLite 数据库。
...
> 不同于旧版本的单节点格式，当前模型以字典键为节点名、值为任务列表。请求体示例：
...
错误记录通过 `append_records` 直接写入 SQLite 数据库，支持高效的查询与分页。

### 任务注入并发安全

`injection_tasks` 字典由 `task_injection_lock` 保护，`push_injection_tasks` 写入和 `pull_task_injection` 读取（含清除）均在锁内操作，避免竞态。注入采用**覆盖**语义：相同节点名的新任务会覆盖旧任务列表。
