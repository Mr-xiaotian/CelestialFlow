...
1. 进入作用域时依次启动全局 `LifecycleSpout` 与 `LogSpout`
2. 退出作用域时按顺序停止 `LogSpout` 和 `LifecycleSpout`
...
3. **停止顺序**：先停止 `LogSpout`，再停止 `LifecycleSpout`，确保日志尽量完整。
