...
| `TaskRedisTransport` | `core_stages` | 预定义节点：将任务推送到 Redis 队列 |
| `TaskRedisSource` | `core_stages` | 预定义节点：从 Redis 队列拉取任务作为输入源 |
| `TaskRedisAck` | `core_stages` | 预定义节点：等待远端 Worker 的执行结果 |
...
3. **预定义节点**: 常见任务模式的实现，如分流器、路由器、Redis 集成等
...
- **与 Persistence 模块**: 通过 `FailInlet` / `LogInlet` 持久化任务状态
