...
> 🟢 本节各表格中的内存/耗时均为历史实测数据，无法从源码验证，需人工确认。

### 历史结果 - Windows 11 容器对比（时间未记录）
...
> 环境：Windows，N=100,000，SHA1 20-byte keys
...
**本轮补充结论**：
- `set[bytes]` 仍然是最紧凑、构建最快的默认选项
- `dict[B,None]` 只多约 25% 内存，若需要更灵活的映射语义，仍是非常划算的折中
- LRU 类结构的主要代价仍然在链表维护与额外节点内存，不在查询速度
