...
| `graph_cpu_pipeline_serial` | 3-node `TaskGraph` 串行 CPU 流水线 |
| `graph_cpu_pipeline_thread` | 3-node `TaskGraph` 线程 CPU 流水线 |
| `graph_io_pipeline_thread` | 3-node `TaskGraph` 线程 I/O 流水线 |

### 测试负载设计

- **CPU 任务**：执行纯 Python 整数循环和哈希式混合运算，尽量压中 Python 字节码执行开销
- **I/O 任务**：`time.sleep()` 模拟阻塞等待
- **图结构**：固定为 3 个 node 的简单串联流水线，避免图拓扑差异干扰结果
