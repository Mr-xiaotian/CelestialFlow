...
1. **默认目标为本地服务**：默认 URL 是 `http://127.0.0.1:5005/api/pull_server_state`。若改用公网 `httpbin.org` 等目标，延迟受网络质量影响，单次结果不具备可重复性。
2. **GIL 限制**：`ThreadPoolExecutor` 中的线程受 Python GIL 约束，`requests` 的 CPU 密集部分（如 TLS 握手、JSON 解析）无法真正并行。
3. **公网目标速率限制**：若指向 `httpbin.org` 等公网服务，频繁测试可能触发限流，返回 429 或连接重置。
...
- 现在的脚本更适合用于验证 celestialflow-web 客户端策略，而不是测公网网络质量
...
### 2026/07/26 - 本地 TaskWebServer（含预热优化）

> 环境：Windows，目标 `http://127.0.0.1:5005/api/pull_server_state`，50 请求，10 并发线程
> 说明：本轮测试基于已加入预热的脚本（每个场景在计时前先发 1 次请求建立连接，并发场景每线程各预热 1 次），结果更稳定
...
**本轮结论**：
- 预热后 Session 复用稳定性显著提升：标准差从前几轮的 1.8–3.1ms 降至 **0.8ms**，说明首请求握手开销已被消除
- 串行场景下 Session 复用收益约 **4.4x**（25.8ms → 5.9ms），且波动极小（max 仅 8.3ms）
- 并发场景下每线程 Session 仍略优于无 Session（34.5ms vs 36.0ms），但差距不大，说明本地接口处理与线程调度已占主导
- 无 Session 的串行结果（25.8ms）比之前几轮（16.0–20.7ms）略高，可能是预热请求占用了单次额外连接后的本机 socket 回收延迟所致，属于正常波动
