...
### 2026/06/16 - 本轮未重跑（外部服务不可用）

> 说明：检查 `127.0.0.1:7777` 与 `127.0.0.1:7778` 时，CelestialTree 的 HTTP / gRPC 端口均不可用

- `bench_http_grpc.py` 依赖本地 CelestialTree 服务，因此本轮未写入新的耗时数据
- 历史结果仍可用于参考协议开销量级，但若要得到当前版本的可比数据，需要先启动对应的 HTTP 与 gRPC 服务端
