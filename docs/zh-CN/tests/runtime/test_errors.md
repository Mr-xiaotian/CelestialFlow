...
| 配置与选项 | 6 | `ConfigurationError`、`InvalidOptionError`（含自定义前缀、`execution_mode` / `graph_mode` / `log_level` 等非法字段值） |
...
- 检查不同非法字段值（如 `execution_mode`、`graph_mode`、`log_level`）均通过 `InvalidOptionError` 统一处理并暴露字段信息。
- 检查默认文案与自定义错误消息是否可读。
