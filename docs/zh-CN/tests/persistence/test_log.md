...
| 类 | 说明 |
|----|------|
| `LogInlet` | 以 `log_level='INFO'` 初始化，提供 `graph_start()` / `task_retry()` / `graph_end()` / `node_start()` 等写入方法 |
| `LogSpout` | 后台线程将队列中的记录批量刷新到日志文件，路径通过 `spout.log_path` 获取 |

## 测试覆盖矩阵

| 测试类 | 用例数 | 覆盖目标 |
|--------|--------|---------|
| `TestLogPersistence` | 1 | 完整日志生命周期：graph_start → task_retry → graph_end → node_start，验证日志文件包含全部内容与等级标记 |

## 关键测试场景

### `test_log_persistence`

- `graph_start("test_graph", "thread", ['test message'])` 写入图启动消息（参数顺序：图名 / 图模式 / 结构列表）。
- `task_retry('func', 'hello world', 1, ValueError('oops'), 0)` 写入带异常信息的 WARNING 级别日志。
- `graph_end("test_graph", 1.0)` 写入图结束事件及耗时。
- `node_start('stage', 1, 'parallel-4')` 写入节点启动记录（节点名 / 任务数 / 执行模式描述）。
