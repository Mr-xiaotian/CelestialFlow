...
| 类 | 说明 |
|----|------|
| `LogInlet` | 以 `log_level='INFO'` 初始化，提供 `start_graph()` / `task_retry()` / `end_graph()` / `start_executor()` 等写入方法 |
| `LogSpout` | 后台线程将队列中的记录批量刷新到日志文件，路径为 `logs/flow_log(<日期>).log` |
...
- `start_graph("test_graph", "thread", ['test message'])` 写入图启动消息（参数顺序：图名 / 图模式 / 结构列表）。
- `task_retry('func', 'hello world', 1, ValueError('oops'), 0)` 写入带异常信息的 WARNING 级别日志。
- `end_graph("test_graph", 1.0)` 写入图结束事件及耗时。
- `start_executor('stage', 1, 'parallel-4')` 写入节点启动记录（节点名 / 任务数 / 模式）。
