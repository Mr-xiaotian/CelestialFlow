...
| `TestDeepGraphRegression` | 3 | 5000 节点深链 / 深环 / 经 TaskGraph 全链路建图 | 深链 SCC 全单点、无 RecursionError；深环收敛为单一 SCC；`get_source_nodes` / `get_graph_analysis` 正常 |
| **合计** | **15** | | |
...
4. **深图回归** (`TestDeepGraphRegression`):
   ...
   - 深链经 `TaskGraph`（`graph_mode="thread"`）全链路建图与分析不崩溃，`layersDict[4999] == ["n4999"]`，`get_nodes()` 返回 5000 个 stage，`get_source_nodes() == ["n0"]`。
