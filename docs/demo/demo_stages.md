...
### `test_redis_ack_0/1/2`
对比 Python 本地计算与通过 Redis + Go Worker 外部计算的耗时差异：
- `test_redis_ack_0`：斐波那契（CPU 密集）
- `test_redis_ack_1`：`sum_int`（通信开销主导）
- `test_redis_ack_2`：图片下载（I/O 密集）
...
1. **Redis 依赖**：`test_redis_*` 系列需要可用的 Redis 服务（`.env` 配置 `REDIS_HOST`、`REDIS_PASSWORD`）。
...
4. **长耗时**：`test_splitter_0` 中各阶段含 4-6 秒随机 sleep，完整执行可能超过 1 分钟。
5. **无断言**：演示脚本，不验证结果正确性。
