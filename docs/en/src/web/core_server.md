...
| Panel | Features |
|-------|----------|
| **Dashboard** | Real-time task graph status overview (structure visualization (Mermaid diagram), node count, success/failure/backlog task counts, line charts) |
...
TaskWeb provides a set of RESTful APIs for `TaskReporter` and frontend use. All endpoints are prefixed with `/api/`; pull endpoints use the `pull_` naming convention, and push endpoints use the `push_` naming convention.
...
| `POST /api/push_status` | Push stage status |
...
| `POST /api/push_history` | Push per-stage history data |
...
class ErrorsMetaModel(BaseModel):
    ...
    rev: int         # Version number (for cache validation)
```
...
class HistoryModel(BaseModel):
    ...
    # key: stage tag; value: [{timestamp, tasks_processed}, ...]
```
...
The web service configuration is persisted in `web/config.json`.

- `load_config()` -- Reads and validates via `WebConfigModel` at startup
- `save_config(config)` -- Saves configuration to a JSON file, thread-safe (uses `_config_lock`)
- `cal_interval(refresh_interval)` -- Converts millisecond refresh interval to seconds, clamped to `[1.0, 60.0]`

When the frontend updates configuration via `push_config`, the `report_interval` is also updated accordingly.
...
- Otherwise, reloads data and updates `_errors_meta_path` / `_errors_meta_rev`
...
Upon receiving this response, the client switches to `push_errors_content` to transmit error content directly.

### Task Injection Concurrency Safety

The `injection_tasks` list is protected by `_task_injection_lock`. Both `push_injection_tasks` writes and `pull_task_injection` reads (with clearing) operate within the lock to prevent race conditions.

## Notes

1. **Port Conflicts**: Ensure the specified port is not in use.
