...
`stage/core_dispatch.py` defines the `TaskDispatch` class, an internal component of `TaskExecutor` responsible for executing individual tasks in serial, thread, or async mode. It fetches tasks from `TaskInQueue`, calls the user function, and sends results through `TaskOutQueue`.
...
class TaskDispatch[T, R]:
    def __init__(
        ...
        :param func: Task function (sync or async)
...
4. If duplicate, call `task_executor.deal_duplicate()` to handle
5. Otherwise, call `_worker()` to execute synchronously
6. Put the merged `TerminationSignal` into `result_queue`
...
2. Fetch tasks from the queue and `submit` to the thread pool
3. Clean up completed futures; continue submitting new tasks when the in-flight count is below `max_workers`
...
Synchronous/asynchronous worker functions, handling a single task with retry support:
...
Retry logic (same in both implementations):
- Execute the task inside `for retry_time in range(max_retries + 1)`
- On success, call `process_task_success` and return
- If exception is in `retry_exceptions` and not yet at `max_retries`, call `log_task_retry()` to log and enter the next loop iteration to retry
- Otherwise, call `handle_task_fail()` to handle failure and return
- If an exception is still thrown outside the inner processing (e.g., an error within `process_task_success` / `handle_task_fail` / `log_task_retry`), the outer `try` catches it and calls `get_log_inlet().worker_crash(e)` to log it without re-raising
...
flowchart LR
    ...
    WORKER -->|success| SUCCESS[process_task_success → write to downstream result_queue]
    WORKER -->|retryable exception| RETRY[log_task_retry → retry in loop]
    WORKER -->|non-retryable exception| FAIL[handle_task_fail → lifecycle/log inlet]
    WORKER -->|outer exception| CRASH[get_log_inlet().worker_crash]
...
flowchart TD
    ...
    DISPATCH -->|calls| TE_METHODS[process_task_success / handle_task_fail / log_task_retry / deal_duplicate]
```
...
`TaskDispatch` is an internal component of `TaskExecutor` and is used indirectly via `TaskExecutor`'s `run()` method.
...
4. **Futures cleanup**: `dispatch_thread` continuously cleans up completed futures, keeping the active task count at no more than `max_workers`
5. **Dedup**: Completed before entering the worker, reducing wasted computation
6. **Retry**: Implemented via a `for retry_time in range(max_retries + 1)` loop inside the worker; no extra retry envelope is sent
