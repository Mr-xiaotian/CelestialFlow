...
- `LifecycleSpout` measures transaction throughput of SQLite lifecycle in "direct `commit()` after each valid change" mode
...
| `LifecycleSpout` | Pre-build `insert` records and push them into the queue, then start `spout` to drain the queue | `start()` to `stop()` |
...
- `--lifecycle-count`: default `20_000`
- `LogSpout` writes to `bench_task_logger.log` in a temporary directory
- `LifecycleSpout` writes to `bench_lifecycle.sqlite3` in a temporary directory
- `LifecycleSpout` uses the current project implementation, i.e., "direct `commit()` after each valid change"
...
2. **`LifecycleSpout`'s bottleneck is primarily transaction commits**: The results largely reflect SQLite `commit()` throughput under the current disk and transaction mode, not just Python loop overhead.
...
> Environment: Windows, local `.venv`, model "pre-populate queue then start spout to drain", `log-count=200000`, `lifecycle-count=20000`

| Test Item | Record Count | Total Time | Throughput | Avg Time per Record |
|--------|--------|--------|------|--------------|
| `LogSpout` | 200,000 | 0.2036s | 982,287.88 records/s | 1.02 us |
| `LifecycleSpout` | 20,000 | 3.2515s | 6,151.08 records/s | 162.57 us |
...
- `LifecycleSpout` is approximately 6.1k records per second, clearly limited by SQLite single-transaction commit
- The throughput gap between the two is approximately **160x**

### 2026/06/17 - Windows local retest (moderately enlarged samples)

> Environment: Windows, local `.venv`, same model, `log-count=300000`, `lifecycle-count=30000`

| Test Item | Record Count | Total Time | Throughput | Avg Time per Record |
|--------|--------|--------|------|--------------|
| `LogSpout` | 300,000 | 0.2960s | 1,013,446.75 records/s | 0.99 us |
| `LifecycleSpout` | 30,000 | 4.6633s | 6,433.23 records/s | 155.44 us |

**Supplementary conclusions for this round**:
- `LogSpout` remains stable at about **1 million records/s** after sample enlargement
- `LifecycleSpout` remains stable at about **6.4k records/s** after sample enlargement
- Under the current machine and implementation, `log` is not the bottleneck; `lifecycle/sqlite` is the primary limiting factor
