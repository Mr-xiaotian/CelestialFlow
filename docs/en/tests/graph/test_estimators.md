...
- `_make_linear_chain(nodes, proc, pend)`: Constructs a linear chain graph A→B→C… and returns a unified processed/pending mapping for reuse in `TestCalcGlobalPending`.
