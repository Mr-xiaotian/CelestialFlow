...
|------|------|
| `connect_db` | Establishes a connection and auto-creates the records table and indexes; automatically adds `retry_times` when an old database is missing the column |
...
| `update_retry_by_event_id` | Updates the retry count and error information of a pending record while keeping the status unchanged; returns `False` when the record does not exist |
...
|--------|--------|---------|
| `TestSpliteUtils` | 19 | Connection & table creation, normalization, insert/read, append/dedup, paginated query, clear, incremental and grouped reads, error type aggregation, state transitions, retry update, deletion, paired reads |
...
- Verifies that the `result_json` field exists, and checks the table structure field order is `id / event_id / ts / stage / status / error_type / error_message / task_json / result_json / retry_times`.
...
- Error records are normalized to `status="failed"`, `retry_times` defaults to 0, and `task_json` is serialized as a JSON string.
...
### State Transitions and Retry Updates
...
- `update_retry_by_event_id`: updates the retry count and error information of a pending record while keeping the status as pending; returns `False` for a non-existent event_id.
