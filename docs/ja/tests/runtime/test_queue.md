...
- `TaskOutQueue`: 複数の下流ノードへのブロードキャストまたはターゲット指定送信を担当。

## 主要テストシナリオ

### `TestTaskInQueue` — 入力キュー
`simple_queue` フィクスチャは、上流ソースを持たない入力キュー（`out_name` のみ）を構築し、単一ソースのケースで再利用されます。
...
出力キューは標準ライブラリ `queue.Queue` を実際の受け皿として使用し、ブロードキャスト/ターゲット指定送信の可視的な結果を検証します。
