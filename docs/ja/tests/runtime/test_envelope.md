...
## 役割
`celestialflow.runtime.core_envelope` モジュールの `TaskEnvelope` クラスと `object_to_hash` ハッシュユーティリティを検証し、タスクデータ、ID、送信元、ハッシュ値が伝送過程で完全性と一貫性を保つことを確認します。
...
| テストクラス | ケース数 | カバレッジ目標 |
|-------------|---------|--------------|
| `TestTaskEnvelope` | 8 | コンストラクタ/Getter、source 保持、ID 変更、ハッシュ一貫性、遅延計算、ハッシュ不可フォールバック、`__slots__` メモリ制限 |
| `TestObjectToHash` | 4 | 戻り値の型(bytes)、SHA1 固定長20バイト、同一入力の一貫性、異なる入力の差異 |

## 主要テストシナリオ

### `TestTaskEnvelope`
1. **基本属性**: コンストラクタパラメータ（task, id, source）が Getter メソッドで正確に復元できることを検証。
2. **ハッシュ一貫性**:
