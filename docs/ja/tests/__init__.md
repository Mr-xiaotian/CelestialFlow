...
- `tests/persistence/`: ライフサイクル sqlite 永続化、ログ永続化、`funnel_scope` スコープ、sqlite ユーティリティテスト。
- `tests/runtime/`: エンベロープ、キュー、ハッシュ、カウンタ、例外、推定テスト。
- `tests/stage/`: TaskStage / TaskExecutor / TaskDispatch および組み込み Stage のテスト。
- `tests/benchmark/`: クローンユーティリティと性能ベンチマークテスト。
