...
| `TaskRedisTransport` | `core_stages` | 定義済みノード：タスクを Redis キューにプッシュ |
| `TaskRedisSource` | `core_stages` | 定義済みノード：Redis キューからタスクを取得し入力ソースとする |
| `TaskRedisAck` | `core_stages` | 定義済みノード：リモート Worker の実行結果を待機 |
...
3. **定義済みノード**: 一般的なタスクパターンの実装。スプリッター、ルーター、Redis 統合など
...
2. **core_stage.py** (`TaskStage`)
   ...
   - **主要機能**:
     ...
     - Inlet バインディング（`set_inlet`）による fail/log キューの永続化層への接続
...
3. **core_stages.py** (定義済みノード: `TaskSplitter`, `TaskRouter`, `TaskRedisTransport`, `TaskRedisSource`, `TaskRedisAck`)
   - **役割**: 一般的なタスクパターンと Redis 統合ノードの事前実装
   - **含まれるノード**:
     - `TaskSplitter`: 入力を複数のサブタスクに分配
     - `TaskRouter`: 条件に応じてタスクを異なる下流ノードにルーティング
     - `TaskRedisTransport`: タスクを Redis キューに送信（言語間・分散実行用）
     - `TaskRedisSource`: Redis キューからタスクを消費（タスクグラフの入力ソースとして）
...
- **Persistence モジュールとの連携**: `FailInlet` / `LogInlet` を通じてタスク状態を永続化
...
graph.connect([router], [pos, neg])  # 2 つの下流にルーティング
