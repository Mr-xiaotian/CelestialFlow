...
1. **dashboard_history.ts**
   - **役割**: 複数指標の履歴シーケンスを管理し、Chart.js で進捗折れ線グラフをレンダリング。指標のリアルタイム切り替えをサポート。

2. **dashboard_statuses.ts**
   - **役割**: 動的ノードカードをレンダリングし、各ステージのリアルタイムパフォーマンス指標とプログレスバーを表示。

3. **dashboard_structure.ts**
   - **役割**: Mermaid.js ベースでタスクグラフトポロジ構造をレンダリング。動的ノード着色をサポート。

4. **injection.ts**
   - **役割**: タスク手動注入 UI を管理。複数ノードの一括注入をサポート。

5. **errors.ts**
   - **役割**: エラーログのページング表示と詳細フィルタリングを担当。
