...
指定された `APIRouter` に全 8 つの POST エンドポイントを登録します。
...
**リクエストボディ:** `StructureModel`（`items` フィールドを含む）

**ロジック:**
1. `data.items` を `server.structure_store` に書き込み
...
グローバルタスクサマリーデータを更新します。

**リクエストボディ:** `SummaryModel`（`summary` フィールドを含む）

**ロジック:**
1. `server.summary_store` を更新
2. `server.store_revs["summary"]` を 1 インクリメント
...
# エラーログをプッシュ（メタモード：サーバーに JSONL を自分で読み取らせる）
