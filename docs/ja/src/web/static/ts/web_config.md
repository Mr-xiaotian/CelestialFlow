...
Web フロントエンドの設定の読み込み、保存、適用を管理します。テーマ、リフレッシュ間隔、ダッシュボードレイアウトを含みます。
...
3. **ダッシュボードレイアウト**: `applyDashboardLayout()` を呼び出し
...
3. `left` / `middle` / `right` の順に、カードを `appendChild` で対応するカラムに移動し、表示状態に設定し、タイトルを更新
...
```json
{
    ...
    "cards": {
        "mermaid": { "title": "タスク構造図" },
        "topology": { "title": "グラフトポロジ情報" },
        "status": { "title": "ステージ実行状態" },
        "progress": { "title": "ステージ完了推移" },
        "summary": { "title": "全体ステータスサマリー" }
    }
}
```
