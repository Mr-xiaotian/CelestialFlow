...
`OrderGraph` は内部で 2 つの隣接リスト（`_in: dict[str, list[str]]` と `_out: dict[str, list[str]]`）を併用してノード集合と辺集合を表現します。`add_node` / `add_edge` はノードを削除せず、ノード走査順序は Python `dict` の挿入順に保証されるため、ノード集合と辺集合は常に安定して保たれます。
