...
class TaskInQueue:
    def __init__(
        ...
        :param maxsize: キューの最大容量。デフォルト 0（無制限）
        """
```
...
def drain(self) -> list[TaskEnvelope]:
    ...
    終了シグナルは記録しますが TerminationIdPool は返しません（同期環境でのみ使用、例: _finish_start）。
    """
```
...
class TaskOutQueue:
    def __init__(
        ...
        """
        :param in_name: 現在のノードの一意名。ログ記録に使用
        """
```

出力キューリストは初期状態で空であり、`add_queue()` を通じて動的に下流チャネルを追加します。
...
def get_target_names(self) -> list[str]:
    """全出力キューのターゲットノード名を取得します。"""
```

現在登録されている全下流チャネルの名前リスト（`_queues` のキー）を返します。
...
# 入力キューを作成し、現在のノード名とキュー容量を指定
in_queue = TaskInQueue(
    out_name="processor",
    maxsize=0,  # 0 は無制限を意味する
)
...
# 出力キューを作成（初期は空、add_queue で動的にチャネル追加）
