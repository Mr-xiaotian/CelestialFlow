...
TaskStageのタスク実行モードは2種類です：

* **シリアル（serial）**
* **マルチスレッド（thread）**

グラフレベルでは、各Stageは3つのコンテキストモードをサポートします：
...
* **プロセス実行（process layout）**：現在のノードを起動後、すぐに次のノードの起動に進みます。
...
- 3.1.8
  - feat:
    - [Important] stage_modeに"thread"モードを追加、I/O集約型タスクやpickle不可の関数に対応;
    - pytest-asyncioサポートを追加、asyncテストが正常に実行可能に;
    - docs/benchとdocs/demoドキュメントを追加、ベンチマーク結果を補充;
    - tests/ディレクトリにテストファイルを追加、旧tests/README.mdを置換;
  - refactor:
    - [Important] connectを組み込みメソッドに変更、set_stagesを追加、root_stageの登録もここで実施;
    - [Important] StageRuntimeを追加、stage実行時ロジックを分離;
    - next_stagesとprev_stagesをin_edgesとout_edgesに置換;
    - stage.set_stage_contextを削除、graph.connectに変更;
    - run_in_*をdispatch_*にリネーム;
    - TaskDispatchのtermination_signal reput機構を廃止;
    - collect_runtime_snapshotを_snapshot_one_stageと_calc_graph_remainに分割;
    - get_binding_counterを追加、prev_bindingsのinstanceof判定を置換;
    - _prepare_task_envelopesを追加、_put_task_queuesロジックを圧縮;
    - successedをsucceededにリネーム、ctree.emitのtype_をCTreeEventに変更;
  - fix:
    - find_unpickleableのbound methodに対する誤検知を修正;
    - stageの重複検出エラーを修正;
    - retryタスクが重複排除されない問題を修正;
  - chore:
    - demo/内のすべてのtest_関数名をdemo_にリネーム;
    - docs構造を再編成、referenceをsrcにリネーム、一部ドキュメントをzh-CN/に移動;
