...
本ディレクトリは `CelestialFlow` プロジェクトの各種 benchmark ドキュメントを収集し、実行モード、グラフスケジューリング、永続化、キュー、ハッシュ、ロックオーバーヘッド、ファネルとロックの同期メカニズム比較、ネットワークリクエスト、および Python 3.14 GIL / No-GIL 比較などのトピックをカバーする。
...
| `bench_requests.md` | HTTP リクエストの再利用と並行戦略のベンチマーク |
...
| `bench_funnel_vs_lock.md` | ファネル（キュー + 単一消費者）とロック（`ValueWrapper`）の同期メカニズム比較 |
