...
### 2026/06/16 - Windows int 負荷再テスト

> 環境：Windows、COUNT=100,000、REPEAT=3、`PAYLOAD_MODE=int`、`SLOT_COUNT=1024`

| トポロジ | MPQueue | SharedMemory ring | 勝者 |
|------|---------|-------------------|--------|
...
**今回の補足結論**：
- 今回 3 つのトポロジすべてで `SharedMemory ring` がリード。履歴で劣っていた MPSC も `MPQueue` を逆転
- 現在のマシンでは複数消費者シナリオの利益が最大で、SPMC では `SharedMemory` のスループットが `MPQueue` の約 3 倍
- これは共有メモリ実装が具体的なマシン、Python バージョン、システム負荷に非常に敏感であることを示しており、履歴結果を対照として保持することには依然として価値がある
