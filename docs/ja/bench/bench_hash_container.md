...
> 環境：Windows、N=100,000、SHA1 20-byte keys
...
**今回の補足結論**：
- `set[bytes]` は引き続き最もコンパクトで構築最速のデフォルトオプション
- `dict[B,None]` はわずか約 25% のメモリ増加で済み、より柔軟なマッピングセマンティクスが必要な場合、依然として非常にコストパフォーマンスの高い妥協案
- LRU 系構造の主要なコストは引き続きリンクリストの維持と追加ノードのメモリにあり、検索速度にはない
