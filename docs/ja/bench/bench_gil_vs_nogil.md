...
2. **スクリプトは作業ディレクトリを切り替える**: `prepare_runtime_root()` が `os.chdir()` をリポジトリルートに切り替えるため、サブディレクトリや一時ターミナルからでも実行可能
...
> 🟢 本節の表にある所要時間はすべて歴史的な実測データであり、ソースコードからは検証できないため、手動での確認が必要です。
