...
2. **GIL 制限**：`ThreadPoolExecutor` 内のスレッドは Python GIL に制約され、`requests` の CPU 集約部分（TLS ハンドシェイク、JSON 解析等）は真の並列化ができない。
3. **公網ターゲットのレート制限**：`httpbin.org` 等の公網サービスを指した場合、頻繁なテストはレート制限をトリガーし、429 または接続リセットが返る可能性がある。
...
- 現在のスクリプトは celestialflow-web クライアント戦略の検証により適しており、公網ネットワーク品質の測定には適さない
...
### 2026/07/26 - ローカル TaskWebServer（ウォームアップ最適化あり）

> 環境：Windows、ターゲット `http://127.0.0.1:5005/api/pull_server_state`、50 リクエスト、10 並行スレッド
> 説明：今回のテストはウォームアップを追加したスクリプトに基づく（各シナリオは計測前に 1 回リクエストを送って接続を確立し、並行シナリオでは各スレッドがそれぞれ 1 回ウォームアップ）。結果はより安定している
...
**今回の結論**：
- ウォームアップ後、Session 再利用の安定性が顕著に向上：標準偏差は従来数ラウンドの 1.8–3.1ms から **0.8ms** に低下し、初回リクエストのハンドシェイクオーバーヘッドが解消されたことを示す
- 逐次シナリオでは Session 再利用の利益は約 **4.4x**（25.8ms → 5.9ms）で、変動も極めて小さい（max はわずか 8.3ms）
- 並行シナリオではスレッドごとの Session が依然として非 Session よりわずかに優れる（34.5ms 対 36.0ms）が、差は小さく、ローカルインターフェース処理とスレッドスケジューリングがすでに支配的であることを示す
- Session なしの逐次結果（25.8ms）は以前の数ラウンド（16.0–20.7ms）よりわずかに高く、ウォームアップリクエストが 1 回余分な接続を占有した後のローカル socket 回収遅延による可能性があり、正常な変動に属する
