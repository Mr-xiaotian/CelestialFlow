...
### 2026/09/23 - ローカル再テスト（Windows / Python 3.14.3）

> 環境：Windows、Python 3.14.3、コマンド `uv run .\bench\bench_execution_mode.py`
...
**今回の補足結論**：
- **CPU シナリオ**：`thread` と `async` はそれぞれ `serial` より約 2.9x と 3.3x 高速である。ここでの加速源は CPU の並列化ではなく、**単一タスクのフレームワークオーバーヘッド**である：`bench_task_1` の中で最大の `n` でも 31 にすぎず、反復アルゴリズムはわずか約 30 回のループで済み、純粋計算の所要時間はマイクロ秒レベルである。そのため測定時間の大部分は各タスクの metrics、ログ/ライフサイクル書き込み（funnel inlet）、および 4 つの異常タスクのリトライ処理で構成される。これらのオーバーヘッドは I/O 寄りの性質であり、`thread`/`async` は worker をまたいで重ねて実行できるが、`serial` は順番に負担するしかない。
- また、`serial` は `benchmark_executor` 内で最初に実行されるモードであり、一回限りのグローバルな spout/lifecycle 初期化オーバーヘッドを追加で負担する可能性があり、これが後続 2 モードとの差をさらに拡大する。したがって今回の CPU データは「マルチスレッドが CPU 計算を加速した」と読んでは**ならない**。
- **I/O シナリオ**：`thread` と `async` はどちらも理論最適の 6x に近く（それぞれ 5.94x / 5.90x）、6 つの `max_workers` による並列上限とほぼ一致する。
- 今回 `thread` は `async` よりわずかに高速（約 0.7%）で、2026/08/18 の macOS 再テスト（`async` がわずかに高速）とは逆であり、**ノイズレベルの差**に属し、このシナリオでは両者は等価と見なせる。
- 2026/08/18 の macOS データと比較する際は注意が必要：環境が macOS から Windows に戻っており、CPU シナリオの絶対値はフレームワーク起動オーバーヘッドの影響を大きく受けるため、2 回の結果を直接比較すべきではない。

## 依存関係

- `celestialflow`（`TaskExecutor`、`benchmark_executor`、およびオプションのオブザーバー `PrintObserver`、本スクリプトでは未有効化）
