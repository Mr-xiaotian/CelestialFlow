...
print("=" * 90)
print(
    f"{'Format':<8} {'Single(B)':>10} {'Total(MB)':>10} {'PerItem(B)':>11} {'Build(ms)':>10} {'Hit(ns)':>10} {'Miss(ns)':>10}"
)
print("-" * 90)
for name in ["str", "bytes", "int"]:
    r = results[name]
    print(
        f"{name:<8} {r['single']:>10} {r['mem_mb']:>10.2f} {r['per_item']:>11.1f} {r['build_ms']:>10.2f} {r['hit_ns']:>10.1f} {r['miss_ns']:>10.1f}"
    )
...
for name in ["bytes", "int"]:
    r = results[name]
    print(
        f"  {name:>5}: total={r['mem_mb'] / base_mem:.1%}  per_item={r['per_item'] / base_per:.1%}"
    )
